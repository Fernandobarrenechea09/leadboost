...
.stat-accent { color: ACCENT_WARM_COLOR; }
...
css = css.replace(“HOT_BG_COLOR”, HOT_BG).replace(“WARM_BG_COLOR”, WARM_BG).replace(“COLD_BG_COLOR”, COLD_BG)
...
wb = Workbook(); ws = wb.active; ws.title = “LeadBoost”
...
with b2:
if leads:
st.download_button(label=“EXPORT XLSX  //”,
data=generate_excel(leads), file_name=“leadboost_leads.xlsx”,
...
accent_class = “ stat-accent” if accent else “”
html = ‘<div class="stat-card">’
html += ‘<div class="stat-label">’ + label + ‘</div>’
html += ‘<div class="stat-value' + accent_class + '">’ + str(value) + ‘</div>’
...
with ch1:
    ...
    if dates:
        ...
        chart = alt.Chart(daily).mark_area(
            ...
            color=alt.Gradient(
                gradient="linear",
                stops=[alt.GradientStop(color=CHART_FILL, offset=0),
            ...
            x=alt.X("date:T", axis=alt.Axis(
                labelFont="JetBrains Mono", labelFontSize=9, labelColor=TEXT_DIM,
                titleColor=TEXT_DIM, domainColor=BORDER, tickColor=BORDER, title=None,
                format="%b %d", labelAngle=0
            )),
            y=alt.Y("Leads:Q", axis=alt.Axis(
                labelFont="JetBrains Mono", labelFontSize=9, labelColor=TEXT_DIM,
                titleColor=TEXT_DIM, domainColor=BORDER, tickColor=BORDER,
                gridColor=BORDER, title=None, tickCount=4
            ))
...
with ch2:
    ...
    bar_chart = alt.Chart(counts).mark_bar(
        color=CHART_LINE, opacity=0.85, cornerRadiusTopLeft=3, cornerRadiusTopRight=3
    ).encode(
        x=alt.X("Rango:N", sort=order, axis=alt.Axis(
            labelFont="JetBrains Mono", labelFontSize=9, labelColor=TEXT_DIM,
            domainColor=BORDER, tickColor=BORDER, title=None, labelAngle=0
        )),
        y=alt.Y("Leads:Q", axis=alt.Axis(
            labelFont="JetBrains Mono", labelFontSize=9, labelColor=TEXT_DIM,
            domainColor=BORDER, tickColor=BORDER, gridColor=BORDER, title=None, tickCount=4
        ))
    ).properties(height=200, background=PANEL).configure_view(strokeWidth=0)
